...
export class AccountService {
  ...
  loadData(login: string) {
    ...
    this.userService.getUserByLogin(login).subscribe(u => {
      this.user = u;
  ...
  userSubscriptions() {
    this.subscriptionService.getSubscriptionsByUserId(this.user.userId).subscribe(s => {
  ...
  allSubscriptions() {
    this.userService.getUsersByRoleId(2).subscribe(u => {
  ...
  createWallet(cardType:string, cardNumber:string) {
    ...
    this.subscriptions.push(this.walletService.saveWallet(wallet).subscribe(w => {
      ...
      this.subscriptions.push(this.userService.addWallet(this.user).subscribe(u => {
        this.hasWallet = true;
        this.user=u;
